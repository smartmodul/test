...
class Rfid:
    ...
    def handle_request(self, rfid_mode, user=None, _id=None):
        if user is not None:
            self.user.user["NAME"] = str(user)
        if _id is not None:
            self.user.user["ID"] = str(_id)
        self.mode = rfid_mode
    ...
    def get_user_from_ID(self, ID: str):
        for user in self.data_layer.users["DATABASE"]:
            if user.get("ID") == ID:
                self.logger.debug(f"RFID with ID={ID}, is already in the database.")
    ...
    def check_name(self, name: str):
        for user_data in self.data_layer.users["DATABASE"]:
            if user_data.get("NAME") == name:
                self.logger.debug(f"Name={name}, is already in the database.")
                return True
        return False

    def add_user(self, name: str, ID):
        if not self.get_user_from_ID(ID):
    ...
    def format_user_row(self, user) -> str:
